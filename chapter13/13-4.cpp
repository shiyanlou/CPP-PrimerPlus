...
private:
	char *brand;
	char style[20];//i.e.,tawny,ruby,vint age
	int bottles;
public: 
	explicit Port (const char *br="none",const char *st="none",int b=0);
	Port (const Port &p);//copyconst ructor
	virtual ~Port (){delete[]brand;}
	Port &operator =(const Port &p);
	virtual void Show()const ;
	Port &operator +=(int b);//addsbtobottles
	Port &operator -=(int b);//subtractsbfrombottles,ifavailable
...
{
	unsigned 
	str_len=strlen(p_src_txt);
	char *
	p_des_txt=new char [str_len+1];
...
{
	unsigned 
	str_len=strlen(p_src_txt)<des_arr_size-1?strlen(p_src_txt): des_arr_size-1;
...
class VintagePort : public Port //stylenecessarily="vint age"
{
	char *nickname;//i.e.,"TheNoble"or"OldVelvet",etc.
	int year;//vint ageyear
	public: 
...
: Port (br,"vint age",b),nickname(cpNewStr(nn)),year(y){}
